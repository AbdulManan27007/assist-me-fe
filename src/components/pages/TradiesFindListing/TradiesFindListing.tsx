...
export function TradiesFindListing() {
  ...
  // Get listings from API with pagination parameters
  ...
  useEffect(() => {
    if (data?.listings?.inactive) {
      let filtered = data.listings.inactive.filter((listing) => {
        return (
  ...
  }, [data?.listings?.inactive, filters, debouncedSearch, limit, page, sortingMethod]);
  ...
  // Refetch data when sortingMethod changes
  useEffect(() => {
    refetch();
  }, [sortingMethod, refetch]);
